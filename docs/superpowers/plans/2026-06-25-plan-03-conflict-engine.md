...
- `@renovatebot/pep440` ^5.0.0 (npm package — pure-JS PEP 440 parser, functional API: `parse()`, `satisfies()`, `validRange()`, `compare()`)
- All other deps from Plan 1/2

> **Correction (post-Task-1-discovery, 2026-06-25):** The original design spec referenced a non-existent `pep440` package. The canonical JS PEP 440 library is `@renovatebot/pep440` v5.0.0 (functional API, no class-based `SpecifierSet`/`Version`). This plan is updated to use it. All downstream tasks (2-10) re-express `parseSpec`/`rangesOverlap`/`intersectRanges` in terms of the functional API.
...
|---|---|---|
| PEP 440 parser | `@renovatebot/pep440` v5.0.0 (functional API) | Only canonical JS PEP 440 library on npm; original spec §6.3's `pep440` package does not exist (404) |
...
### Task 1: Add `@renovatebot/pep440` dependency

**Files:**
- Modify: `web/package.json` (add `@renovatebot/pep440` to `dependencies`)
- Modify: `web/pnpm-lock.yaml` (lockfile regenerated by pnpm)

**Interfaces:**
- Consumes: none (additive change)
- Produces: `import { parse, satisfies, validRange, compare } from '@renovatebot/pep440'` available throughout `web/lib/**`

**Goal:** Install the `@renovatebot/pep440` npm package so subsequent tasks can use it.

- [ ] **Step 1: Install the package**

```bash
cd web && pnpm add @renovatebot/pep440
```

Expected: `@renovatebot/pep440` appears in `web/package.json` `dependencies`, lockfile updated.

- [ ] **Step 2: Verify install**

```bash
cd web && pnpm ls @renovatebot/pep440
```

Expected: prints `@renovatebot/pep440 5.0.x`.

- [ ] **Step 3: Verify import works**

```bash
cd web && pnpm exec node -e "const p = require('@renovatebot/pep440'); console.log('parse:', typeof p.parse, 'compare:', typeof p.compare, 'satisfies:', typeof p.satisfies);"
```

Expected: prints `parse: function compare: function satisfies: function` (all three are functions).

- [ ] **Step 4: Commit**

```bash
git add web/package.json web/pnpm-lock.yaml
git commit -m "chore(deps): add @renovatebot/pep440 npm package for conflict engine"
```
...
**Notes:**
- Use `@renovatebot/pep440` v5.0.0 (functional API). Key functions:
  - `parse(version: string)` → returns parsed version object (or throws on invalid)
  - `compare(a: string, b: string)` → returns -1, 0, 1 (works on version strings, no need to parse first)
  - `satisfies(version: string, range: string)` → boolean (true if version is in range)
  - `validRange(range: string)` → returns the range if valid, null otherwise
- The functional API does NOT expose class-based `SpecifierSet` or `Version` constructors. The utilities in this file are pure-function wrappers — they extract `min`/`max`/`isPinned` from a spec string by simple string parsing (split on `,`, then regex each part for `>=`/`<`/`==`/etc.). `compare()` is then used for the actual version comparison.
- `isPinned` means a single `==X.Y.Z` spec (not a range with `,`).
- For `intersectRanges`: parse all specs (taking the max of all `min` and the min of all `max`); if `max < min`, return null. Mark `isPinned` if any spec is pinned.
- The simpler approach: don't try to extract `min`/`max` from the spec string ourselves — store the spec as-is, and use `satisfies()` to check inclusion. But the plan's interface returns `{min, max, isPinned}` so the implementer can choose either approach.
...
export function intersectRanges(
  ...
  for (const p of parsed) {
    if (!p.min) continue;
    if (!maxMin || compare(p.min, maxMin) > 0) maxMin = p.min;
  }
  let minMax: string | null = null;
  for (const p of parsed) {
    if (!p.max) continue;
    if (!minMax || compare(p.max, minMax) < 0) minMax = p.max;
  }
  if (maxMin && minMax && compare(maxMin, minMax) > 0) return null;
  const isPinned = parsed.some((p) => p.isPinned);
  return { min: maxMin ?? '0', max: minMax ?? '', isPinned };
}
```

> **Note for implementer:** The `@renovatebot/pep440` package's `compare()` function works directly on version strings — no need to parse first. The test cases above are the source of truth — they must all pass.
