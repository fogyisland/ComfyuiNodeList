...
- ✅ **wiki_revisions.version_id FK drift resolved (2026-08-05, migration `20260805_normalize_wiki_revisions_version_id_fkey`)**: dev DB had `ON DELETE CASCADE ON UPDATE CASCADE` for `wiki_revisions_version_id_fkey` despite the `20260626_wiki_revisions_no_action` migration being marked applied. Root cause likely an out-of-band `prisma db push` against dev DB at a point where `schema.prisma` still had `onDelete: Cascade`, before the no_action migration ran. Test DB and fresh DB already had the correct FK. New idempotent migration added: `DROP FOREIGN KEY` + `ADD CONSTRAINT … ON DELETE NO ACTION ON UPDATE NO ACTION`. Applied to all 4 DBs (dev, test, fresh, shadow); all verified `NO ACTION`. `prisma migrate diff` returns empty; gating check holds.
